...
PersonListElem* sortInsert(PersonListElem *first, PersonListElem* cur, Person* ref) {
    PersonListElem *new_person = new PersonListElem();
    new_person->elem = ref;
    if (first == nullptr || first->elem->age < cur->elem->age) {
        new_person->next = first;
        first = new_person;
    }
    else {
        PersonListElem *pl;
        for (cur = first; cur != nullptr && cur->elem->age >= ref->age; cur = cur->next){
            pl = cur;
        }
        new_person->next = cur;
        pl->next = new_person;
...
PersonListElem* sortList(PersonListElem* list) {
    PersonListElem *cur;
    PersonListElem *empty = nullptr;
    for (cur = list; cur != nullptr;) {
        empty = sortInsert(empty, cur, cur->elem);
        PersonListElem *tmp = cur;
        cur = cur->next;
        delete tmp;
    }
