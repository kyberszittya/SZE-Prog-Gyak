 ...
 * Ez a módszer gyorsan összerakható és egyszerűen megjegyezhető,
 * de túlzottan egyszerű módon generál számokat és gyakran detereminisztikus
 * (pszeudo-random -> determinisztikus generálás)
 */
double monteCarloPi(unsigned int n) {
    // Old random number generation
    int bent = 0;
    for (int i = 0; i < n; i++) {
